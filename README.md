...
**v0.2.0** (unreleased) — pre-1.0: the API is stable in shape but not frozen, and 0.2.0 changed that shape substantially (composition is `co_await`; `then`/`when_all`/task builders/retraction are gone). See [CHANGELOG.md](CHANGELOG.md) for the full list. Some areas are actively evolving (**WIP**): the allocation/performance campaign, a platform abstraction layer, and benchmark regression tracking. See [docs/TODO.md](docs/TODO.md) for the live roadmap.
