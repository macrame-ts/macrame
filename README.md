...
- **Some functionality is uncommon or unique**: a `co_await`-a-guard model where holding an access guard across a suspension is *detected and fails fast*; runtime **deadlock detection** for the suspended-cycle case that occupies no thread (a waits-for cycle check plus a global quiescence net); scheduler's configurable idle policies; and object hand-off between graph nodes.
...
**v0.1.0** (unreleased) — pre-1.0: the API is stable in shape but not frozen. Composition is `co_await`; there are no `then`/`when_all`/task builders. See [CHANGELOG.md](CHANGELOG.md) for what 0.1.0 contains. Some areas are actively evolving (**WIP**): the allocation/performance campaign, a platform abstraction layer, and benchmark regression tracking. See [docs/TODO.md](docs/TODO.md) for the live roadmap.
