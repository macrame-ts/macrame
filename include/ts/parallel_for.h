...
inline Scheduler& current_or_default_scheduler() noexcept
{
    return current_scheduler ? *current_scheduler : default_scheduler();
}

template<typename Body>
Parallel_state<Body>* make_parallel_state(int n, Body&& body, Parallel_options opts, int& conc_out)
{
    int conc = opts.concurrency > 0 ? opts.concurrency : current_or_default_scheduler().worker_count();
    if (conc < 1)
        conc = 1;
    if (conc > n)
        conc = n;
...
void parallel_for(int n, Body&& body, Parallel_options opts = {})
{
    ...
    Scheduler& sched = detail::current_or_default_scheduler();
