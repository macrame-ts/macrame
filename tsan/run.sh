...
# std::move_only_function / std::jthread / std::counting_semaphore (libstdc++ 14+
# or a recent libc++). TSan has NO Windows runtime -- run this on Linux/macOS
# (e.g. WSL or a container), from anywhere:
#   bash tsan/run.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
"$CXX" -std=c++23 -fsanitize=thread -fno-exceptions -O1 -g -pthread \
    -I. -Isample $SRC -o tsan/ts_tsan

TSAN_OPTIONS="halt_on_error=1 second_deadlock_stack=1" ./tsan/ts_tsan
