...
**A C++23 parallelisation framework built around controlled access to shared data.**

A comprehensive task system — work-stealing scheduler, dependency graphs, coroutines, cancellation, data-parallel loops — with one thing no other C++ task system has at its centre: **you declare which shared data each piece of work touches, and the framework derives safe parallelism from those declarations**, backed by a runtime harness that catches any access you forgot to declare.

Inspired by game engines — high performance, soft real-time frame budgets, many interacting subsystems sharing state — but built to fit any system with those characteristics. No external dependencies.
...
- Multiple **separate internal parts** — subsystems, "data islands" — each thread-unsafe on its own (physics, animation, audio, AI; or a server's services, a simulation's modules).
- **Intensive communication and data sharing** between those parts — not an embarrassingly-parallel loop over uniform data.
...
You can already build this by hand with mutexes, atomics, and an existing task library. The goal here is to make it **safer and more straightforward** — to remove a large, recurring class of data-race and ordering bugs, and the effort of policing them, from that work.

**The high-level-parallelism bet.** Express parallelism where the *surface area is smallest* — between coarse parts, not deep inside every algorithm. Once enough parallelism is expressed at that level to keep all CPU cores busy, low-level parallelisation stops paying off and you can skip it. Less low-level threading means far less complexity, less code, and fewer bugs. This framework is built to make high-level parallelism easy to express **and** safe.

**Probably not for you** if your problem is a single tight data-parallel kernel, one shared container, or fully independent jobs with no shared state — a plain thread pool or `std::for_each(par)` will serve you better and simpler.
...
This framework separates them. You **declare the data each task accesses and how** — read or write, from a functor's parameter const-ness — and the framework **derives** the ordering and exclusion needed for safety. A **runtime harness** then verifies, as work runs, that nothing touched shared state it did not declare, aborting with a stack trace instead of racing. Dependencies remain available for genuine ordering; they are no longer overloaded to mean "these tasks share data."

```cpp
// The whole model in one line: the parameter's const-ness IS the access declaration.
inventory.async([](Inventory& inv)       { inv.add(sword); });    // T&        -> write, exclusive
inventory.async([](const Inventory& inv) { return inv.count(); }); // const T&  -> read, concurrent
```
...
- **Access orchestration is a first-class, novel feature.** `Guarded<T>` + the runtime harness give you a thread-safe API for a shared object with the safety *checked*, not merely conventional. To our knowledge no other C++ task system derives scheduling from declared data access and polices it at runtime. This is the centre of the library.
- **It is also a comprehensive, state-of-the-art task system**, not a thin wrapper: work-stealing scheduler (Chase-Lev deques + lock-free MPMC queues + an eventcount), typed continuations and joins, cooperative cancellation with mid-body early-out, reusable and nested tasks, priorities, coroutine integration, and data-parallel loops.
- **Some functionality is uncommon or unique**: staged-write layers (`Deferred`/`Versioned`) that let many producers record changes lock-free and apply them atomically at a chosen point; **retraction** (a blocking wait runs not-yet-started work inline, so fork-join can't deadlock the pool); configurable idle policies (UE-style spin-then-park, Go-style handoff); object hand-off between graph nodes; and a `co_await`-a-guard model where holding an access guard across a suspension is *detected and fails fast*.

For a feature-by-feature comparison with UE Tasks, Taskflow, TBB, HPX, Folly, Go, and others, see [docs/task-systems-comparison.md](task-systems-comparison.md).

---

## Examples

The examples build up from a single guarded object to the full frame. All snippets use the real API.

### 1. A thread-safe API for one object — with coroutines

Wrap a thread-unsafe object in `Guarded<T>`; reach it only by declaring access. With coroutines, that declaration reads as ordinary linear code — `co_await` suspends until access is granted, then hands you a guard with direct, harness-checked access:
...
{
    auto inv = co_await ts::write(inventory);   // suspend until EXCLUSIVE access; no thread blocked
...
{
    auto inv = co_await ts::read(inventory);    // SHARED read access — concurrent with other readers
    co_return inv->count();
}
```
...
Layered — use as much as you need. The access model (`Guarded` + the harness) is the core; everything else builds on it.

- **`Guarded<T>`** — a thread-safe API for a shared object: a per-object reader/writer pipe (concurrent reads, exclusive writes, FIFO, non-blocking submit), plus multi-object operations with deadlock-free ordered acquisition.
- **Tasks** — `launch` work with prerequisites (`after`), continuations (`then`), typed joins (`when_all`), cooperative cancellation (incl. mid-body early-out), reusable and nested tasks, and priorities. Blocking waits run not-yet-started work inline (retraction), so fork-join can't deadlock the pool.
- **`Static_task_graph`** — build-once/run-many DAG whose edges are derived from access conflicts (plus explicit ordering where you want it); re-runs are allocation-free.
- **`Deferred<T>` / `Versioned<T>`** — staged writes: record grant-free from any thread, apply the batch atomically at a defined point; `Versioned` gives readers a whole-frame stable snapshot. Deterministic by construction.
...
- **`parallel_for`** — for the data-parallel work that does live inside a part; caller-participating (nested-safe), with guided/balanced/unbalanced chunking.
- **Scheduler** — work-stealing (Chase-Lev deques + lock-free MPMC queues + eventcount), configurable idle policies, priorities. Used through the layers above; reach for it directly only when you need to.
...
- A **340-check test suite**, plus subprocess **death tests** for every fatal path.
...
Two real concurrency bugs found and fixed by this tooling — including one that survived four rounds of happens-before analysis — are written up in [docs/design.md](design.md).

---

## Requirements & building

C++23, no external dependencies, exceptions disabled project-wide (failures are fatal-by-design). Compilers: MSVC and clang-cl on Windows; clang on Linux.
