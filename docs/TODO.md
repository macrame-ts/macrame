...
1. **Guarded / access**
   ...
   *Standing priority (author, 2026-07): anything that strengthens the harness
   ...
   8. `[x]` **(P1) `Access_options` vs `Dispatch_options` - DONE (2026-08-23, API-stability M2(b)).** Split, not shared: `Launch_options` was renamed `Dispatch_options` `{token, priority, name}` and taken by every verb that always schedules (`ts::launch`, `Guarded::async`, the free `ts::async`, `Deferred::commit`, `Versioned::publish`), while `Access_options` = the same three plus `queued` and is taken only by the opportunistic verbs (`Guarded::access`, the free `ts::access`, `Versioned::read`, the `Access_op` ctors), which are the only ones with an inline arm to skip. No inheritance: a base subobject cannot be named by a designator, so `Access_options{.priority = p}` would stop compiling (verified against MSVC 19.51: `error C7559`). `Versioned::publish` also stopped dropping `opts.name`. Was: — revisit whether the two option aggregates share a base or stay split (flagged in `task.h`); resolve in the code-review API-stability pass.
...
2. **Static task graph**
   ...
   17. `[x]` **(P2, author 2026-08 — DONE at the C4 review, 2026-08-17) `execute()` in a loop
       ...
       checked builds (and races the `Run_state` with checks off), while the returned handle is
       silently discarded. Discuss whether the fatal is the right UX or the API should guide harder.
       Candidate directions: `[[nodiscard]]` on the return so a dropped run is at least flagged; a
   ...
   19. `[ ]` **(P2, author 2026-08-18 - from the C4 lend review, item 14) Harness check: caller
       ...
       guide state it).
3. **Scheduler**
   0. `[x]` **Single global scheduler — DONE (2026-07, `d173d9a`+`2b48a5b`; lifecycle API reworked 2026-08).** Author chose single-global over the ambient-multi model (3.1). Exactly one scheduler process-wide, reachable via `global_scheduler()`. The lifecycle is EXPLICIT, not lazy — a scheduler is heavy, so the subsystem-init shape applies (`SDL_Init`/`SDL_Quit`): `ts::create_scheduler(Scheduler_config = {})` brings the pool up (fatal if one already runs), `ts::destroy_scheduler()` signals quit, joins the workers and drains (fatal if none runs), `ts::scheduler_running()` answers whether one is up, `ts::current_scheduler_config()` returns the config it was created with, and `Scheduler_scope(config)` is the RAII form (creates, or reconfigures by teardown+recreate if one is already running, and restores the previous config on exit) — the way to run a block on a specific worker count / idle policy. `global_scheduler()` FATALS when none is running rather than lazily constructing one, so a missing `create_scheduler` is a sharp startup error instead of a silently default-configured pool. All of these are coarse quiescent-point operations, not thread-safe against concurrent use. `execute()` drops its `Scheduler&` arg (uses the global); `parallel_for` fans out on `global_scheduler()`. Supersedes 3.1's ambient-override idea and its `Launch_options{.scheduler}` (nothing to select among). **Residual (P3): compile-time ban on ad-hoc `ts::Scheduler` — DONE (2026-08).** The `Scheduler` ctor is PRIVATE; the sole construction path is a `detail::make_scheduler(config)` factory (friend; returns `unique_ptr`, class non-movable) used by the global holder, so `ts::Scheduler s{...}` no longer compiles. Went further than the original plan: rather than migrate the unit tests to the factory (which would still stand up rival pools), every ad-hoc site (`scheduler_tests.cpp`, benchmarks, tsan, `parallel_tests.cpp`) reconfigures the GLOBAL for its scope (`Scheduler_scope`), so at most one worker pool ever runs. The `thread_local current_scheduler` "which scheduler" selector was retired entirely — `current_worker_index >= 0` is the sole worker-vs-external discriminator (fast path, `parallel_for` fan-out, profiling bridges, `Parallel_recorder::lane`); `current_or_global_scheduler()` collapsed to `global_scheduler()`. Verified: Release, Debug and Shipping suites green, bench in-band, --stress deterministic, TSan clean.
   ...
   **Two distinct oversubscription mechanisms (UE, verified from source — keep them separate, both are SCHEDULER-impl concerns behind the unchanged `Task` API, NOT task-layer):** (i) *background-band gap-filling* — the variant above: a standing low-OS-priority pool the OS preempts out the instant foreground work is ready. (ii) *dynamic oversubscription around a blocking region* — `FOversubscriptionScope` bumps a count that wakes a parked standby worker (or spawns one under dynamic thread creation, `IncrementOversubscription`); surplus workers self-retire via `ConditionalStandby` when the count drops. (ii) keeps the pool alive when a task genuinely BLOCKS (mutex / I/O / `Wait` on unrelated work) — the case our current scheduler does NOT cover. (2026-08: awaits no longer belong on that list — a `co_await` suspends the frame and frees the worker, and retraction is gone. What is left uncovered is opaque blocking inside a body, with no suspension point for us to use; see 5.3.) (ii) is the prior art for closing that gap in the alternative scheduler; see 5.3.
   ...
   9. `[ ]` **(P1, evidence 2026-08-18) Investigate the 2x-worker throughput collapse.** Across two
      ...
      run with `num_workers = 2 x hw` collapses to a near-constant ~0.5-0.63 M tasks/s - regardless
      ...
      scheduler, not the workload; prime suspect: with more workers than cores, some workers are
      always parked, so every producer `submit` sees sleepers and pays a wake syscall
      (`signal_submit`), capping the flood producer at roughly 1/wake-cost - but that is a
      hypothesis, not a diagnosis. Matters beyond benches: any user configuring workers > cores
      (the oversubscription mitigation this library tells mutex users they no longer need) hits it.
...
5. **Fork-join / parallel_for**
   ...
   4. `[ ]` **(P2, author 2026-08 — raise at the parallel_for review) Reconsider `parallel_for` inheriting the caller's grant (esp. a WRITE grant).** The helpers all carry the node's grant, so each passes `TS_CHECK_ACCESS` on the shared object - but the harness does NOT serialize helpers against each other. Data-race-freedom rests entirely on the disjoint-partition user contract (helper k writes disjoint slices), exactly like `std::for_each(std::execution::par)`. A body that writes a *shared* location (accumulating into one member, an overlapping range) races silently, undetected by the harness - the completeness/granularity footgun, but now manufactured by the fan-out. Revisit: is grant-inheritance the right model; is a guard/diagnostic for overlapping writes feasible (hard in general - it is the parallel-for aliasing problem); or is documenting the disjointness contract sufficient? Raised during the C2 `task.h` review (parallel_for inherits via `snapshot_access` + `Inherited_access_scope`, joins synchronously so the grant window stays open).
...
10. **Tooling / infra**
    ...
    15. `[x]` **(P1-cheap, CI) Gate Shipping (`TS_SAFETY_CHECKS=0`) on every push — DONE.**
        ...
        behavioral dependency only shows as a Shipping build/link/behaviour break. `.github/
        workflows/ci.yml` runs a `windows-shipping` job (clang-cl, the `windows-shipping` CMake
        preset) on every push to `master` and every pull request: configure, build, `--tests`,
        with the exe/pdb/crash dump uploaded as artifacts on failure. `--stress` stayed on the
        TSan leg rather than duplicating into the Shipping one.
...
### D3. Ambient scheduler — SUPERSEDED (see entry 0)
Superseded by the single-global choice: exactly one process-wide pool, reconfigured via
`configure_scheduler`/`Scheduler_scope`, with construction factory-gated and the
`thread_local current_scheduler` selector retired (2026-08). The multi-pool / ambient-override
direction below was not taken. Kept for the rationale record only.
