...
1. **Guarded / access**
   ...
   3. `[x]` **(P1) Generic-lambda / `auto&` deduction — DONE.** True auto-deduction from `auto&` is impossible (a generic lambda's `operator()` is a template with no introspectable parameter const-ness, and the body compiles against both `T&` and `const T&`), so the fix is an explicit per-object mode tag: `ts::as_read(g)` / `ts::as_write(g)` (in `guarded.h`). `add_node` / multi-object `ts::access`/`ts::async` accept either all-bare args (const-ness deduced, unchanged) or all-tagged args (mode explicit, works with a generic lambda); mixing is a `static_assert`. Names `as_read`/`as_write` avoid colliding with the coroutine guards `read_only`/`read_write`. **Naming — author ratify:** alternatives were `read`/`write` (short but were just vacated by the guard rename → confusing) and `reads`/`writes`; `as_*` chosen as clearest. Tests: `graph_tests` (generic node ordering + reader overlap), `integration_tests` (multi-object generic basic + exclusion). 345→356 checks; clean MSVC + clang-cl, TSan clean.
