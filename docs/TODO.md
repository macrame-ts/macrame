...
1. **Guarded / access**
   ...
   3. `[x]` **(P1) Generic-lambda / `auto&` deduction — DONE.** True auto-deduction from `auto&` is impossible (a generic lambda's `operator()` is a template with no introspectable parameter const-ness, and the body compiles against both `T&` and `const T&`), so the fix is an explicit per-object mode tag: `ts::as_read_only(g)` / `ts::as_read_write(g)` (in `guarded.h`). `add_node` / multi-object `ts::access`/`ts::async` accept either all-bare args (const-ness deduced, unchanged) or all-tagged args (mode explicit, works with a generic lambda); mixing is a `static_assert`. Tags are also usable with NON-generic functors for those preferring explicit declaration over `const` deduction (the tag then wins; no cross-check against the parameter's const-ness — a mismatch surfaces via the harness at runtime). Naming ratified (author, 2026-07): `as_read_only`/`as_read_write`, matching the `Access` enum and the coroutine guards `read_only`/`read_write` (originally shipped as `as_read`/`as_write`; renamed for vocabulary consistency). Tests: `graph_tests` (generic node ordering + reader overlap), `integration_tests` (multi-object generic basic + exclusion). 345→356 checks; clean MSVC + clang-cl, TSan clean.
