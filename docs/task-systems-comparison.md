...
- **TLS context** holds the active scheduler *and* the worker's local queue (a richer version of this project's `current_worker_index`, which — with one process-wide pool — is all that is needed to route a worker's own submits to its local deque), so a task launched from within a worker enqueues locally with no global contention.
...
Tying the survey back to the codebase as it stood at the 2026-06 baseline (single shared `priority_queue` + mutex, single-class workers, intrusive refcount, `block`/`spin` policy). These lessons drove the work that followed — items 1, 3 and 5 are now implemented (per-priority lock-free MPMC queues + per-worker Chase-Lev deques, the `co_await` blocking story, and spin-then-block idle), and item 2's zero-alloc instinct was broadened into monomorphic single-allocation task blocks and the zero-alloc `Access_op` (a tunable-SBO callable for the residual heap cases is still open); see [design.md](design.md) §3. Kept as the research record that motivated them:
...
3. **Decide a blocking story early.** Options, cheapest-to-adopt first: continuation chaining (the `then` of the day), oversubscription, or fibers. Fibers (Marl/ND) give the nicest call sites but are a large commitment; continuations compose with the existing dependency model with no new runtime machinery. (Settled since: the coroutine-first transformation made `co_await` the one composition mechanism and an in-task blocking `sync()` a fatal — see design.md §4.3–4.4.)
